...
- `<leader>uga` - 현재 버퍼에 최근 변경 시각 색 막대 토글
  - 1일 이내: 빨강 계열
  - 1주 이내: 노랑 계열
  - 1달 이내: 파랑 계열
- 동작 방식: `git blame` 결과를 바탕으로 각 라인의 마지막 커밋 시각을 계산하여, 좌측 텍스트 컬럼의 가장 왼쪽에 얇은 막대(▏)를 표시합니다. Gitsigns의 사인 컬럼은 건드리지 않습니다.
- 팁: 대용량 파일이나 저장소에서 처음 토글 시 잠시 시간이 걸릴 수 있습니다.
